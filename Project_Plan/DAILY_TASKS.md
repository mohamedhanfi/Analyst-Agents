...
- **Last finished task:** Task 5 — Stage 4 Cleaning — ✅ done. New `shared/core/cleaning.py` (strategy table §2.4 incl. auto `drop_negative`, Python-authoritative normalize, versioned attempts) + `shared/tools/cleaning.py` (7 wrappers) + `agents/cleaning_agent.py` (3 tasks, `run_cleaning(run_dir, use_crew=…)`, CLI `--crew`, recheck cap 3 → `cleaning_retry_limit_exceeded`). Live deterministic runs both green: 7-row `run_20260811_034636_1` (temporal cast, attempt 1) and `sales_demo.csv` (202→198: 2 dups + 2 negatives dropped, attempt 1, DQ recheck passed); `253 passed in ~14s`
- **Last commit:** `9a1b259` (MK, "fix(stage-2): role rules …", **not pushed**) — Tasks 3+4 work sits in pushed `b0168ef`; Task 5 = **uncommitted** (see working tree)
- **Baseline tests:** 253 passing in `tests/unit/` (94 contracts + 8 ingestion + 55 understanding + 35 data quality + 7 data-quality tools + 6 role-rules regression + 32 cleaning core + 12 cleaning tools + 4 cleaning agent)
- **Working tree:** 4 files uncommitted for Task 5 — `shared/core/cleaning.py` · `shared/tools/cleaning.py` · `agents/cleaning_agent.py` · `shared/tools/__init__.py` + 3 test files `test_cleaning_{core,tools,agent}.py` — **commit before Task 6**; `.env` is local + gitignored (OpenRouter key)
- **Open items / decisions:**
  - Task 6 Analysis (**next task**, Stage 5a): consume `cleaned_data.csv` + `dataset_understanding.json`; growth executor defaults `basis` → `previous_period` when absent (handoff row 1); `analysis/evidence.py` is **the only writer** of `evidence_registry.json`
  - Stage 4 crew live run (`--crew`) deferred — needs the user's real terminal + OpenRouter key (deterministic path fully verified)
